...
def pcd_prep(pcd):
    downS_cplt_S1 = pcd.voxel_down_sample(voxel_size =0.08)
    cl, ind = downS_cplt_S1.remove_statistical_outlier(nb_neighbors=100, std_ratio=2.0)
    return cl
...
def floor_plan_extraction_as_is(pcd_data):
    ...
    xy_surf = xy_cloud.delaunay_2d(alpha=0.2)
    xy_edges = xy_surf.extract_feature_edges(boundary_edges=True,
                           feature_edges=False,
                           manifold_edges=False)
    xy_surf.plot(cpos="xy", show_edges=True)
    shape_area = xy_surf.area
    xy_count = len(xy_view.index)
    centr_x, centr_y = xy_view["x"].sum()/xy_count, xy_view["y"].sum()/xy_count
    xy_p = pv.Plotter()
   ...
   # return file_svg
    ...
    while type(lines) != np.ndarray:
        n1 = random.randint(0,500)
        n2 = random.randint(0,500)
        n3 = random.randint(0,500)
        lines = cv2.HoughLinesP(img_edges, 1,math.pi / 180.0, n1, minLineLength=n2, maxLineGap=n3)
    ...
    while i < 5:
       ...
       # p = pv.Plotter()
        ...
        i=i+1 
    return (xy_surf, xy_edges,median_angle, shape_area, centr_x, centr_y)
